...
3. **Two fixture files per eval kind.** Inputs and human-curated golden truth are separate files joined by the slug — see the existing pairs (`<slug>.search.json` + `<slug>.golden.json`, `<slug>.expand-input.json` + `<slug>.expand-golden.json`). When adding a kind, follow the suffix convention.
...
5. Create `scripts/run-<kind>.js` modeled on [run-expand.js](scripts/run-expand.js) — load fixtures, build ctx, call the strategy, render report, `writeRun(...)`.
