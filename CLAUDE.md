...
- **Storage schema**: defined inline in each adapter — see [src/adapters/storage/sqlite.js](src/adapters/storage/sqlite.js) and [src/adapters/storage/indexeddb.js](src/adapters/storage/indexeddb.js). Logical tables: `events`, `event_views` (per-saved-query junction recording which events were actually shown to the user — distinct from `events`, which holds everything curation produced), `preferences`, `kv`, and `saved_queries` for user-defined searches keyed on `(city, queryText)`. Cross-session dedupe filters by `event_views` membership (via `getShownIds`), not by mere presence in `events`. The pipeline never auto-marks events shown — consumers populate `event_views` by calling `curator.markShown(ids, { city, queryText })` when the events actually become visible to the user (e.g. the TUI calls it per page rendered).
- **Rank stage (filter + rank unified)**: there is no separate filter stage. Rank strategies may both drop events and reorder them. `rules` is the rule-based exclude (`excludeKeywords` / `excludeVenues` / price bounds); `byDate` reorders chronologically; `llmRank` is a combined filter + rank LLM pass that omits poor matches and attaches a ~5-word rationale. Default chain is `[rules, byDate]`; the TUI uses `[rules, llmRank]`. Don't reintroduce a separate filter stage.
