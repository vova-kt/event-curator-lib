...
- **searchByQueries** (`searchByQueries(ctx, query, opts?)` → `{ hits, queries, usage }`) — [src/stages/searchByQueries.js](../src/stages/searchByQueries.js). Runs `ctx.strategies.queryExpansion` concurrently (`Promise.allSettled`), unions + lower-cases the queries, then fans every `(adapter, query)` pair across `ctx.search` in parallel (`Promise.all`). Hits dedup on a canonical URL form (lowercased scheme+host, `www.` stripped, fragment removed, common tracking params dropped, trailing slash trimmed) so the same page returned with different tracking suffixes collapses to one hit. Per-`(adapter, query)` failures are logged and isolated; an empty `queryExpansion` array is a misconfiguration and throws.
- **searchRankSources** (`searchRankSources(hits, ctx, query, opts?)` → `{ hits, usage }`) — [src/stages/searchRankSources.js](../src/stages/searchRankSources.js). Runs `ctx.strategies.searchRankSources` in order on the `SearchHit[]` from searchByQueries. Strategies may shrink or reorder the hit list. Per-strategy failures are logged and isolated; the stage is a no-op when the strategy array is empty. See [strategies.md](strategies.md).
