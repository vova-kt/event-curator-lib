...
Strategies are pluggable functions. Four kinds: `queryExpansion`, `searchRankSources`, `dedupe`, `rank`. They live in [src/strategies/](../src/strategies/) and are passed to `createCurator` as arrays — applied in order. There is no separate `filter` kind for events; rule-based excludes and LLM-based soft filtering are both rank strategies, since the filter stage was folded into rank (see [pipeline.md](pipeline.md)). `searchRankSources` operates on `SearchHit[]` before extraction — a different concern.
...
[src/strategies/queryExpansion/](../src/strategies/queryExpansion/). The searchByQueries stage runs all configured strategies concurrently, lower-cases + trims for case-insensitive dedup, and fans the union out across search adapters. A single strategy that throws is logged and skipped; others continue. An empty strategy *array* is a misconfiguration and `searchByQueries` throws.
...
- **`llmExpand({ limit } = {})`** — one LLM call (`expandQueriesPrompt`) producing diverse queries: synonyms / sub-genres, narrow timeframe phrasings (specific days, weekends, dates inside the window — not just broad month names), and balanced coverage across three language registers when applicable: English, the city's primary local language (e.g. German for Berlin), and any language implied by the user's query itself (e.g. Russian for "russian standup"). `limit` defaults to `config.queryExpansion.defaultLimit`. Results are persisted in storage `kv` under key `qx:llmExpand:v2|<city>|<queryText>|<from>|<to>` so the same triple skips the LLM on repeat. The `:v2` suffix lets us bust the cache by bumping the version when the prompt or key shape changes.
...
[src/strategies/searchRankSources/](../src/strategies/discoverFilter/). Runs inside the searchByQueries stage after URL-dedup, before hits flow to extract. Operates on `SearchHit[]` — can score, filter, and enrich hits (e.g. fetch full page content). Default in `createCurator`: `[]` (no-op, opt-in). Config tunables live under `config.searchRankSources` in [src/core/config.js](../src/core/config.js).

- **`searchRelevanceStrategy()`** — LLM relevance scoring + full-content fetch. Sends all hit URLs, titles, and snippets to the LLM via `scoreRelevancePrompt`, gets a 0–1 composite score per hit based on URL signal, title relevance, and snippet relevance. Drops hits below `config.searchRankSources.relevanceThreshold`, takes the top `config.searchRankSources.maxFetch`, then fetches full page content for those hits concurrently using `@mozilla/readability` + `linkedom`. Failed fetches fall back gracefully (the hit passes through with its original snippet). Returns only the top N hits — strict filtering.
